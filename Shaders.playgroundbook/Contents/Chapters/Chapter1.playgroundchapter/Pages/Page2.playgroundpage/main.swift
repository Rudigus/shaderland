...
/*:
 ...
 The Vertex Shader stage, as you may have guessed, is the stage where we work with the vertices of the object we want to draw. In shaders, we can think of vertices as geometry's vertices, so we would need eight vertices to draw a cube, for example. At this stage we can change attributes of a vertex, like position and color. I think it will all become clear with an example!
 ...
 **Task**: Deform a Cube
 ...
 4. Finally, try changing the second line to `  _geometry.position.x += 0.5 * sin(u_time);`. Can you guess how does it work?
 */
//#-hidden-code
...
/*:
 * Callout(Explanation):
   How does the code above work? Considering that our objective in this playground is understanding shaders in general, the logic behind the code is more important than the way it is written. Here is a transcription of the logic used for the 3rd step of *Deform a Cube*:
    ...
    For clarification of this transcription, it's useful to know some values. The scene's origin, where the position is 0 for both x and y axes, is located right at the center of the original, undeformed cube. Also, the original cube has sides of length 1.
 
    Transcription of the 4th step:
 
    "First, check if the position on the y-axis of some vertex is greater than 0. If it is, increase the vertex's position on the x-axis by 0.5 multiplied by the sine of the time (in seconds) since the shader started to be used for rendering. Otherwise, leave the vertex unmodified."
 
    Using periodic functions alongside u_time is a staple in shader programming. Since u_time keeps increasing as the time passes and periodic functions keep repeating themselves as their input increases, the combo is extremely suitable to animate attributes over time.
